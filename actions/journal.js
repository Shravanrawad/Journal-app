...
export async function createJournalEntry(data) {
  try {
    ...
    const entry = await await db.entry.create({
      data: {
        title: data.title,
...
export async function deleteJorunal(id) {
  try {
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized");
...
export async function updateJorunal(data) {
  try {
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized");
    ...
    const updetedentry = await await db.entry.update({
      where: {id: data.id},
      data: {
        title: data.title,
        content: data.content,
