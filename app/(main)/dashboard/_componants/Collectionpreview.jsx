...
function Collectionpreview({id, name, entries = [], isUnorganized = false, isCreateNew =  false, onCreateNew }) {
  ...
  return (
    <Link className='group relative' href={`/collection/${isUnorganized ? 'unorganized' : id}`}>
           ...
           <div className={`relative rounded-lg p-6 shadow-md hover:shadow-lg transition-all ${colorSchemes[isUnorganized ? "unorganized" : 'collection'].bg}`}>
                ...
                <div className='space-y-2'>{entries.length > 0 ? (
